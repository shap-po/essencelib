...
void main()
{
    ...
    float holeDist = clamp((dist - 0.2) / (0.3 - 0.2), 0.0, 1.0);
    ...
    color -= 0.2 * vec3(0.0, 0.0, timeInSeconds);
    ...
    vec3 emissionColor = emission(gradientColor.rgb, noiseColorLength * 0.4);
    ...
    vec3 facVec = vec3(fac);
    vec4 resColor = mix(vec4(emissionColor, vertexColor0.a), vec4(facVec, 0), fac + 1.2);
    resColor = mix(vec4(0.0, 0.0, 0.0, vertexColor0.a), resColor, holeDist);
