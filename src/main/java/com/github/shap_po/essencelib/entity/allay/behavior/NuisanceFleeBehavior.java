...
public class NuisanceFleeBehavior extends ExtendedBehaviour<AllayEntity> {
    public NuisanceFleeBehavior() {
        this.startCondition(allay -> {
            ...
            return focus != null && NuisanceAllayHelper.isPlayerChasing(allay, focus, world.getTime());
        });
    }
    ...
    protected void tick(AllayEntity allay) {
        ...
        if (NuisanceAllayHelper.isPlayerChasing(allay, focus, world.getTime())) {
            NuisanceAllayHelper.runFleeBehavior(allay, focus, world.getTime());
        }
    }
